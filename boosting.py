...
class XGBoostTimeSeriesForecaster:
    """
    A time series forecaster using XGBoost.
    """
    ...
    def __init__(self, n_estimators=100, max_depth=6, learning_rate=0.1, 
        ...
        self.model = xgb.XGBRegressor(
            ...
            max_bin=256,                # Controls histogram bin size
    ...
    def load_taxi_data(self, file_path, sample_frac=1.0, max_rows=None):
        """
        ...
        try:
            ...
            # Take a random sample to reduce memory footprint if specified
            if sample_frac < 1.0:
                df = df.sample(frac=sample_frac, random_state=42)
                print(f"Sampled {sample_frac*100}% of data")
            
            # Further limit to max_rows if specified
    ...
    def create_time_series_features(self, df, target_col, 
        ...
        for window in window_sizes:
            ...
            df_copy[f'{target_col}_roll_min_{window}'] = df_copy[target_col].rolling(window=window).min()
            df_copy[f'{target_col}_roll_max_{window}'] = df_copy[target_col].rolling(window=window).max()
            
            # Calculate rate of change
            df_copy[f'{target_col}_roc_{window}'] = df_copy[target_col].pct_change(periods=window)
            
            # Add exponentially weighted features
            df_copy[f'{target_col}_ewm_{window}'] = df_copy[target_col].ewm(span=window).mean()
    ...
    def train_evaluate(self, X_train, X_test, y_train, y_test, early_stopping_rounds=50):
        """
        ...
        """
        print(f"Training XGBoost model with {X_train.shape[0]} samples and {X_train.shape[1]} features")
        
        # Create DMatrix objects for XGBoost (optional but can be more efficient)
        ...
        params = {
            ...
            'tree_method': 'hist',  # Fast histogram algorithm
            'grow_policy': 'lossguide',  # More efficient for large datasets
            'max_bin': 256,  # Controls histogram bin size
            'seed': self.model.random_state
        }
        
        # Train model using the XGBoost native API for more control
        bst = xgb.train(
            params,
            dtrain,
    ...
    def cross_validation(self, X, y, n_splits=5, gap=0, early_stopping_rounds=50):
        """
        ...
        params = {
            'objective': 'reg:squarederror',
            'max_depth': self.model.max_depth if hasattr(self.model, 'max_depth') else 6,
            'learning_rate': self.model.learning_rate if hasattr(self.model, 'learning_rate') else 0.1,
            'subsample': self.model.subsample if hasattr(self.model, 'subsample') else 0.8,
            'colsample_bytree': self.model.colsample_bytree if hasattr(self.model, 'colsample_bytree') else 0.8,
            'tree_method': 'hist',
            'seed': self.model.random_state if hasattr(self.model, 'random_state') else 42
        }
        ...
        for i, (train_idx, test_idx) in enumerate(tscv.split(X)):
            ...
            bst = xgb.train(
                params,
                dtrain,
                num_boost_round=200,  # Adjust as needed
                evals=eval_list,
                early_stopping_rounds=early_stopping_rounds,
                verbose_eval=False
            )
            ...
            print(f"Fold {i+1} RMSE: {rmse:.4f}, Best Iteration: {bst.best_iteration}")
    ...
    def plot_feature_importance(self, top_n=20, importance_type='gain'):
        ...
        Args:
            ...
            importance_type: Type of importance to plot ('weight', 'gain', 'cover')
        """
    ...
    def plot_learning_curve(self):
        """
        Plot learning curve from evaluation history.
        """
        if not hasattr(self.model, 'evals_result'):
            print("No evaluation history available. Train the model first with eval_set.")
            return
        ...
        # Plot training curve
        if 'train' in results:
            plt.plot(results['train']['rmse'], 'b-', label='Training RMSE')
        
        # Plot validation curve
        if 'test' in results:
            plt.plot(results['test']['rmse'], 'r-', label='Validation RMSE')
    ...
    def load_model(self, filepath):
        ...
        # Create booster
        bst = xgb.Booster()
        bst.load_model(filepath)
        self.model = bst
        print(f"Model loaded from {filepath}")


def main():
    """Main function to demonstrate XGBoost time series forecasting."""
    
    # Initialize the forecaster with parameters optimized for larger datasets
    forecaster = XGBoostTimeSeriesForecaster(
        n_estimators=200,  # More estimators for the full dataset
        max_depth=8,       # Slightly deeper trees
        learning_rate=0.05, # Lower learning rate for better generalization
        subsample=0.8,
        colsample_bytree=0.7, # Lower column sample to avoid overfitting
        random_state=42
    )
    
    # Load and prepare data - using full dataset
    # Replace this path with your actual file path
    df = forecaster.load_taxi_data('taxi-dataset.parquet', sample_frac=1.0)
    ...
    # Train and evaluate the model - use more early stopping rounds for larger dataset
    results = forecaster.train_evaluate(X_train, X_test, y_train, y_test, early_stopping_rounds=50)
